  ...
  return (
    <header ref={navRef} className="fixed top-0 left-0 right-0 z-50 bg-parchment/90 backdrop-blur-sm border-b border-gold/20">
      <div className="max-w-7xl mx-auto px-6 h-16 flex items-center justify-between">
        <a href="#" className="font-serif text-xl living-gradient font-bold">
          Mundo de Colores
        </a>

        <nav className="hidden md:flex items-center gap-8">
          {links.map(l => (
            <a key={l.href} href={l.href} className="text-sm font-medium text-charcoal/70 hover:text-charcoal transition-colors">
              {l.label}
            </a>
          ))}
          <a
        ...
        <div className="md:hidden bg-parchment border-t border-gold/20 px-6 pb-4">
          {links.map(l => (
            <a key={l.href} href={l.href} onClick={() => setMenuOpen(false)}
              className="block py-3 text-charcoal/80 hover:text-charcoal border-b border-gold/10 last:border-0">
              {l.label}
            </a>
          ))}
