...
export default function PaymentResult() {
  ...
  function downloadUrl(code: string) {
    return '/api/download-book?payment_id=' + encodeURIComponent(paymentId || '') + '&book_code=' + encodeURIComponent(code)
  }
  ...
  return (
    <main className="flex min-h-screen items-center justify-center bg-parchment px-5 py-12">
      <section className="w-full max-w-lg rounded-3xl bg-white p-8 text-center shadow-xl sm:p-12">
        {isChecking && <LoaderCircle className="mx-auto animate-spin text-gold" size={48} />}
        {isApproved && <CheckCircle2 className="mx-auto text-sage" size={52} />}
        {!isChecking && !isApproved && <TriangleAlert className="mx-auto text-gold" size={52} />}
        ...
        {isApproved && paymentId && (
          <div className="mt-8 flex flex-col gap-3">
            {items.map(item => (
              <a key={item.code} href={downloadUrl(item.code)} className="inline-flex w-full items-center justify-center gap-2 rounded-full bg-charcoal px-6 py-4 font-semibold text-parchment transition-colors hover:bg-gold">
                <Download size={18} />
        ...
        {isApproved && (
          <a href="/" className="mt-4 inline-flex w-full items-center justify-center rounded-full border border-charcoal/15 px-6 py-3.5 text-sm font-semibold text-charcoal transition-colors hover:bg-charcoal/5">
            Volver a la página principal
          </a>
        )}

        {!isChecking && !isApproved && (
          <a href="/" className="mt-8 inline-flex rounded-full bg-charcoal px-6 py-3 text-sm font-semibold text-parchment transition-colors hover:bg-gold">
            Volver a la tienda
          </a>
        )}
      </section>
